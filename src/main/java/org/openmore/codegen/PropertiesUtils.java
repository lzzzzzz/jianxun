...
public class PropertiesUtils {
    ...
    public static String getPropertyValueByKey(String propertyFile, String key) throws Exception{
        if (null != propertyFile && !"".equals(propertyFile)){
            ...
            String fType = propertyFile.substring(index + 1, propertyFile.length());
            if ("xml".equals(fType)){
                //xml解析
                System.out.println("开始解析xml");
                return getPropertyValueXml(propertyFile, key);
            }
            if ("properties".equals(fType)){
                //properties解析
                throw new Exception("非法解析格式(" + fType + ")=>目前仅支持xml、properties格式配置解析");
            }
            throw new Exception("非法解析格式(" + fType + ")=>目前仅支持xml、properties格式配置解析");
        } else{
        ...
        try{
            ...
            if (nodes.getLength() == 0){
                ...
                for (int i = 0; i < nodes.getLength(); i++){
                    ...
                    String keyStr = e1.getAttribute("key");
